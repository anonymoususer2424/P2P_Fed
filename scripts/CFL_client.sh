...
function EXEC() {
    ...
    while [ "$i" -lt "$n" ]; do
        CID=$((i+abs_i))
        PID=$(( helper_port+50+i*50 ))
        CMD="python -u fedavg_client.py -p $PID -P $helper_port -t $CID -c $s -g 0 -A $helper_ip -a $this_addr -i -m $model -d $dataset"
        sleep 1
        echo "$CMD"
        PORT=$((PID+49))
        log=$proj_dir/log/CFL_${CID}.log
        date > $log
